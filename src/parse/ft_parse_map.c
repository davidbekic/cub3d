...
/*   Updated: 2023/05/19 14:57:20 by dbekic           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_parse_map(t_data *d, char *path) {
    ...
    worldMap = malloc(d->map.height * sizeof(int *));
    for (int i = 0; i < d->map.height; i++) {
        worldMap[i] = malloc((d->map.width + 1) * sizeof(int)); // +1 for null terminator
    }
    ...
    while (read(fd, buf, 1) > 0) {
        if (buf[0] == '\n') {
            // worldMap[row][col] = '\0'; // null terminate the string
            row++;
            col = 0;
        } else {
            if (buf[0] == 'N')
